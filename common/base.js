...
function updateTime() {
    ...
    for (const item of time_update_list) for (const element of document.querySelectorAll(item.selector)) element.innerText = d.slice(item.start, item.end)
}
...
function addStorageField(id, selector, name, placeholder, filter = x = >x, callback = () = >{}) {
    ...
    for (const element of elements) element.innerText = filter(init_value),
    ...
    for (const _item of fields[id]) for (const _element of document.querySelectorAll(_item.selector)) _element.innerText = _item.filter(res || _item.placeholder)
});
    callback(init_value)
}
// ToHome
function navigateHome() {
    window.location.href = "https://jackjen8.github.io/jacker.github.io/"
}
// ToTripCard
